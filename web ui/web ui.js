...
const WARRANTY_OPTIONS = [
  { id: 'w1', years: 1, price: 0, coverage: 'Standard manufacturer warranty' },
  { id: 'w2', years: 2, price: 79, coverage: 'Extended coverage + accidental damage protection' },
  { id: 'w3', years: 3, price: 149, coverage: 'Full coverage + free repairs + annual screen replacement' }
];

/* ===== GUARANTEE & DISCOUNT DATA ===== */
const GUARANTEES = {
  'credit-card': { name: 'Credit Card Guarantee', discount: 5, benefits: ['5% cashback reward', 'Price protection', 'Extended warranty coverage'] },
  'debit-card': { name: 'Debit Card Guarantee', discount: 3, benefits: ['3% instant rebate', 'Fraud protection', 'Extended return period'] },
  'cash': { name: 'No additional discount', discount: 0, benefits: ['Standard return policy'] }
};

const RAFFLE_MULTIPLIERS = {
  'credit-card': 5,
  'debit-card': 2,
  'cash': 1
};
...
const PRODUCTS = {
  ...
  tabs: [
    ...
    { id:'t2', brand:'Samsung', name:'Galaxy Tab S10 Ultra', price:1099, oldPrice:1349, rating:'★★★★★ 4.8 (2.2k)', img:'../images/Samsung%20Galaxy%20Tab%20S10%20Ultra.webp', deal:'-19%', specs:{Display:'14.6" Dynamic AMOLED',Chip:'Snapdragon 8 Gen 3',Storage:'256GB',Battery:'11200mAh'}, colors:['#2c2c2c','#e8e8e8'], desc:'14.6-inch Galaxy AI powerhouse. Dual front cameras, S Pen included, 11200mAh battery.' },
    ...
    { id:'t4', brand:'Lenovo', name:'Tab P12 Pro', price:649, oldPrice:799, rating:'★★★★☆ 4.4 (678)', img:'../images/Lenovo.webp', deal:'-19%', specs:{Display:'12.6" AMOLED',Chip:'Snapdragon 870',Storage:'256GB',Battery:'10200mAh'}, colors:['#2a2a2a','#e0e0e0'], desc:'Premium AMOLED for entertainment and productivity. JBL quad speakers, 10200mAh battery.' },
    ...
    { id:'t6', brand:'Google', name:'Pixel Tablet 2', price:599, oldPrice:699, rating:'★★★★☆ 4.3 (521)', img:'../images/Google%20Pixel%20Tablet%202.webp', deal:'-14%', specs:{Display:'11" LCD',Chip:'Tensor G3',Storage:'256GB',Battery:'7020mAh'}, colors:['#c4a882','#2c2c2c','#4a6b5a'], desc:'Intelligent tablet with Gemini AI. Doubles as smart home hub with charging speaker dock.' },
  ],
  accessories: [
    { id:'a1', brand:'Apple', name:'AirPods Pro 3', price:249, oldPrice:299, rating:'★★★★★ 4.9 (8.9k)', img:'../images/Air%20pods%20pro%203.jpg', deal:'-17%', specs:{Type:'TWS Earbuds',ANC:'Adaptive',Battery:'30h total',Connect:'Bluetooth 5.4'}, colors:['#f0f0f0','#1c1c1e'], desc:'World\'s most popular earbuds. H2 chip delivers best-in-class ANC and Transparency mode.' },
    { id:'a2', brand:'Sony', name:'WH-1000XM6', price:349, oldPrice:399, rating:'★★★★★ 4.9 (6.1k)', img:'../images/Sony%20WH.jpg', deal:'-13%', specs:{Type:'Over-Ear',ANC:'Industry Best',Battery:'30h',Connect:'Bluetooth 5.3'}, colors:['#1a1a1a','#d4c4b0','#2d4a7a'], desc:'Industry-leading noise canceling. Auto NC Optimizer, 30-hour battery, Precise Voice Pickup.' },
    { id:'a3', brand:'Apple', name:'Apple Watch Ultra 3', price:799, oldPrice:899, rating:'★★★★★ 4.8 (3.4k)', img:'../images/Apple%20Watch%20Ultra%203.jpg', deal:'-11%', specs:{Display:'49mm LTPO OLED',Chip:'S10',GPS:'Dual-Freq L1+L5',Battery:'72h'}, colors:['#d4d0c8','#e8e0d0'], desc:'Most capable Apple Watch. Titanium case, dual-frequency GPS, 72 hours battery for athletes.' },
...
let selectedWarranty = 'w1';
let raffleTicketsEarned = [];
...
function openModal(p) {
  ...
  const warrantyHtml = WARRANTY_OPTIONS.map(w => 
    `<div class="warranty-option"><input type="radio" name="warranty" value="${w.id}" ${w.id==='w1'?'checked':''} onchange="selectedWarranty=this.value"><strong>${w.years}-Year</strong> ${w.price > 0 ? `+$${w.price}` : '(Included)'} - ${w.coverage}</div>`).join('');
  
  const guaranteeHtml = `<div class="guarantee-badge"><i class="fas fa-shield-alt"></i> ${p.guarantee || 'Full satisfaction guaranteed'}</div>`;

  const warranty = WARRANTY_OPTIONS.find(w => w.id === selectedWarranty);
  const totalPrice = p.price + (warranty ? warranty.price : 0);
  ...
  content.innerHTML = `
    ...
    <div class="modal-info-col">
      ...
      <div class="modal-price">$${totalPrice} ${p.oldPrice ? `<span class="modal-old">$${p.oldPrice}</span>` : ''}</div>
      ...
      <div class="modal-spec-title"><i class="fas fa-lock"></i> Warranty Protection</div>
      <div class="warranty-options">${warrantyHtml}</div>
      <button class="modal-add-btn" onclick="addToCart('${p.id}');closeModal()">
        <i class="fas fa-shopping-bag"></i> Add to Cart — $${totalPrice}
      </button>
    </div>`;
...
function addToCart(id) {
  ...
  const itemData = { 
    ...product, 
    qty: 1,
    warranty: { id: selectedWarranty, years: warranty.years, price: warranty.price },
    warrantyDesc: warranty.coverage
  };
  if (existing) {
    existing.qty++;
  } else {
...
function renderCartDrawer() {
  ...
  itemsEl.innerHTML = cart.map(item => `
    <div class="cart-item-d">
      ...
      <div class="cart-item-d-info">
        ...
        <div class="warranty-badge-small"><i class="fas fa-shield-alt"></i> ${item.warranty.years}-yr warranty</div>
        <div class="dp">$${(item.price * item.qty).toLocaleString()} + ${item.warranty.price > 0 ? '$' + (item.warranty.price * item.qty) : 'warranty incl.'}</div>
        <div class="qty-ctrl">
  ...
  // Calculate totals with warranty
  const subtotal = cart.reduce((s,i) => s + (i.price + i.warranty.price) * i.qty, 0);
  const shipping = subtotal > 999 ? 0 : 9.99;
  const discount = GUARANTEES[selectedPaymentMethod];
  const discountAmount = +(subtotal * discount.discount / 100).toFixed(2);
  const tax = +((subtotal - discountAmount) * 0.08).toFixed(2);
  const grandTotal = +(subtotal - discountAmount + shipping + tax).toFixed(2);
  
  // Calculate raffle tickets
  const raffleMultiplier = RAFFLE_MULTIPLIERS[selectedPaymentMethod];
  const ticketsEarned = Math.floor(subtotal / 50) * raffleMultiplier;
  raffleTickets += ticketsEarned;

  const paymentOptionsHtml = Object.entries(GUARANTEES).map(([key, value]) => `
    <label class="payment-option">
      <input type="radio" name="payment" value="${key}" ${selectedPaymentMethod === key ? 'checked' : ''} onchange="selectedPaymentMethod='${key}';renderCartDrawer()">
      <span class="payment-label">
        <strong>${value.name}</strong>
        <small>${value.benefits.join(' • ')}</small>
      </span>
    </label>
  `).join('');
  ...
  footerEl.innerHTML = `
    <div class="payment-methods-section">
      <div class="cart-summary-row" style="font-weight:bold;margin-bottom:8px"><span><i class="fas fa-credit-card"></i> Payment Method</span></div>
      ${paymentOptionsHtml}
    </div>
    <div class="raffle-section" style="background:#f0f0f0;padding:8px;border-radius:6px;margin:8px 0">
      <div class="cart-summary-row"><strong>🎰 Raffle Tickets</strong><span>${ticketsEarned} earned</span></div>
      <small style="color:#666">Every $50 spent = free raffle ticket</small>
    </div>
    <div class="cart-summary-row"><span>Subtotal</span><span>$${subtotal.toLocaleString()}</span></div>
    ${discount.discount > 0 ? `<div class="cart-summary-row" style="color:var(--accent)"><span>${discount.name} (-${discount.discount}%)</span><span>-$${discountAmount}</span></div>` : ''}
    ...
    <button class="checkout-btn-d" onclick="processCheckout()">
      <i class="fas fa-lock"></i> Checkout Securely
    </button>`;
}

function processCheckout() {
  const orderSummary = `✅ Order Confirmed! Payment: ${GUARANTEES[selectedPaymentMethod].name} | Raffle Tickets: ${raffleTickets} 🎰`;
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        // observer.unobserve(entry.target); // Keep observing if we want to repeat, or unobserve for performance
      }
    });
  }, observerOptions);
