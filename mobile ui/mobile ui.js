...
const WARRANTY_OPTIONS = [
  ...
  { id: 'w2', years: 2, price: 79, coverage: 'Extended + accidental damage' },
  { id: 'w3', years: 3, price: 149, coverage: 'Full coverage + free repairs' }
];

/* ===== GUARANTEE & DISCOUNT DATA ===== */
const GUARANTEES = {
  'credit-card': { name: 'Credit Card', discount: 5, benefits: ['5% cashback', 'Price protection'] },
  'debit-card': { name: 'Debit Card', discount: 3, benefits: ['3% rebate', 'Fraud protection'] },
  'cash': { name: 'No discount', discount: 0, benefits: ['Standard policy'] }
};
...
const PRODUCTS = {
  phones: [
    {
      ...
      img: '../images/Apple-iPhone-16-Pro.jpg',
      deal: '-14%',
    ...
    },
    {
      ...
      img: '../images/Samsung%20Galaxy%20S25%20Ultra.jpg',
      deal: '-15%',
    ...
    },
    {
      ...
      img: '../images/pixel%209%20pro%20xl.jpg',
      deal: '-8%',
    ...
    },
    {
      ...
      img: '../images/xiaomi-15-pro-1.jpg',
      deal: '-18%',
    ...
    },
    {
      ...
      img: '../images/Sony%20Xperia%201%20VI.webp',
      deal: '-13%',
  ...
  tabs: [
    ...
    },
    {
      ...
      img: '../images/Samsung%20Galaxy%20Tab%20S10%20Ultra.webp',
      deal: '-19%',
    ...
    },
    {
      ...
      img: '../images/Google%20Pixel%20Tablet%202.webp',
      deal: '-14%',
  ...
  accessories: [
    {
      ...
      img: '../images/Air%20pods%20pro%203.jpg',
      deal: '-17%',
    ...
    },
    {
      ...
      img: '../images/Apple%20Watch%20Ultra%203.jpg',
      deal: '-11%',
    ...
    },
    {
      ...
      img: '../images/Samsung%20Galaxy%20Buds3%20Pro.jpg',
      deal: '-17%',
...
function openDetail(p) {
  ...
  const warrantyOptions = WARRANTY_OPTIONS.map(w => 
    `<div class="warranty-chip ${w.id === 'w1' ? 'active' : ''}" onclick="selectedWarranty='${w.id}'; updateDetailWarrantyUI()">${w.years}yr ${w.price > 0 ? '+$'+w.price : ''}</div>`
  ).join('');

  const getPriceWithWarranty = () => {
    const w = WARRANTY_OPTIONS.find(x => x.id === selectedWarranty);
    return p.price + (w ? w.price : 0);
  };
  ...
  container.innerHTML = `
    ...
    <div class="detail-body">
      ...
      <div class="detail-price" id="detail-price-display">
        $${getPriceWithWarranty()}
      ...
      <div class="detail-section"><i class="fas fa-lock"></i> Warranty Protection</div>
      <div class="warranty-chips">${warrantyOptions}</div>
      <div class="detail-section">Description</div>
      <div class="detail-desc">${p.desc}</div>
      <button class="detail-add-btn" onclick="addToCartMobile(${JSON.stringify(p).replace(/"/g,'&quot;')})">
...
function updateDetailWarrantyUI() {
  detailProduct = detailProduct || {};
  const warranty = WARRANTY_OPTIONS.find(w => w.id === selectedWarranty);
  const totalPrice = detailProduct.price + (warranty ? warranty.price : 0);
...
/* ===== CART ===== */
function addToCartMobile(product) {
...
function renderCart() {
  ...
  listEl.innerHTML = cart.map(item => `
    <div class="cart-item">
      ...
      <div class="cart-item-info">
        ...
        <div class="warranty-small" style="font-size:11px;color:var(--accent);margin:4px 0"><i class="fas fa-shield-alt"></i> ${item.warranty.years}-yr warranty</div>
        <div class="pprice">$${((item.price + item.warranty.price) * item.qty).toLocaleString()}</div>
        <div class="qty-ctrl">
  ...
  // Calculate with warranty and discounts
  const subtotal = cart.reduce((sum, i) => sum + (i.price + i.warranty.price) * i.qty, 0);
  ...
  const paymentOptionsHtml = Object.entries(GUARANTEES).map(([key, value]) => `
    <div class="payment-chip ${selectedPaymentMethod === key ? 'active' : ''}" onclick="selectedPaymentMethod='${key}';renderCart()">
      <strong>${value.name}</strong>
      <small>${value.benefits[0]}</small>
    </div>
  `).join('');

  summaryEl.innerHTML = `
    <div style="padding:12px;background:rgba(0,212,170,0.1);border-radius:8px;margin-bottom:12px">
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:8px"><i class="fas fa-credit-card"></i> Payment Method</div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">${paymentOptionsHtml}</div>
    </div>
    <div style="background:rgba(245,158,11,0.1);border-left:3px solid var(--accent3);padding:8px;margin-bottom:12px;border-radius:4px;font-size:12px">
      <strong>🎰 Raffle Tickets: ${ticketsEarned}</strong><br><small>Every $50 = 1-${raffleMultiplier} tickets</small>
    </div>
    <div class="summary-row"><span>Subtotal</span><span>$${subtotal.toLocaleString()}</span></div>
    ${discount.discount > 0 ? `<div class="summary-row" style="color:var(--accent)"><span>${discount.name} (-${discount.discount}%)</span><span>-$${discountAmount}</span></div>` : ''}
    ...
    <button class="checkout-btn" onclick="processCheckoutMobile()">
      <i class="fas fa-lock"></i> Checkout Securely
    </button>
  `;
}

function processCheckoutMobile() {
  showToast(`✅ Order Confirmed! Raffle Tickets: ${raffleTickets} 🎰`);
  cart = [];
  raffleTickets = 0;
  renderCart();
  goTo('screen-home');
